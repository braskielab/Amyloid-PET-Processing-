...
#-----------------------------------------------------------------
echo "STAGE 9: STEP 1 -->  ${subj} CALCULATE ROI ${ROI} MEAN SUVR (REF ADJUSTED)"
#PET_MEAN=$(${FSLSTATS} ${WEIGHTED}/${subj}_PET_ROI_${ROI}.nii.gz -M)
#REF ADJUSTED >
PET_MEAN=$(${FSLSTATS} ${WEIGHTED}/adj/${subj}_adjPET_ROI_${ROI}.nii.gz -M)
echo -e "STAGE 9: STEP 1 -->  ${subj} CALCULATE ROI ${ROI} MEAN SUVR = ${PET_MEAN} \r\n" >> ${NOTE}

#-----------------------------------------------------------------
#STEP (2) - INSERT PET VALUES BY ROI
#-----------------------------------------------------------------
echo "STAGE 9: STEP 2 -->  INSERT ${subj} ROI MEAN SUVR IN CSV FILE"
