...
#------------------------------------------------------------------
echo "STAGE 7: STEP 1 --> Running ${REGIONEXTRACT}"
${REGIONEXTRACT} ${T1}/${subj}_aparc+aseg_to_native.nii.gz ${LABEL} ${WEIGHTED}/${subj}_T1_${ROI}.nii.gz

# STEP (2)
echo "STAGE 7: STEP 2 --> Binarizing ${subj} extracted ${ROI} region"
...
#------------------------------------------------------------------
echo "STAGE 7: STEP 3 --> ${subj} FS ROIs TRANSFORMED FROM T1 NATIVE SPACE TO PET PET"
cmd="${FSLFLIRT}
...
eval ${cmd}
touch ${NOTE}
echo -e "STAGE 7: STEP 3 --> ${subj} FS ROIs TRANSFORMED FROM T1 NATIVE SPACE TO PET PET \r\n" >> ${NOTE}
echo -e "COMMAND -> $cmd\r\n" >> ${NOTE}

#------------------------------------------------------------------
#STEP (4) - THRESHOLD & BINARIZE EXTRACTED PET ROI
#------------------------------------------------------------------
echo "STAGE 7: STEP 4 --> ${subj} PET ROI ${ROI} BINARIZE"
cmd="${FSLMATHS} ${WEIGHTED}/${subj}_PET_${ROI}.nii.gz -thr .5 -bin ${WEIGHTED}/${subj}_PET_${ROI}_bin.nii.gz"

eval ${cmd}
touch ${NOTE}
echo -e "STAGE 7: STEP 4 --> ${subj} PET ROI ${ROI} BINARIZE  \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}
...
#------------------------------------------------------------------
echo "STAGE 7: STEP 5 -->  ${subj} ROI ${ROI} REGION MASK MULTIPLIED TO PET BET"
cmd="${FSLMATHS} ${PET_OUT}/${subj}_PET_BET.nii.gz -mul ${WEIGHTED}/${subj}_PET_${ROI}_bin.nii.gz ${WEIGHTED}/${subj}_PET_ROI_${ROI}.nii.gz"

eval ${cmd}
touch ${NOTE}
echo -e "STAGE 7: STEP 5 -->  ${subj} ROI ${ROI} REGION MASK MULTIPLIED TO PET  \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}
